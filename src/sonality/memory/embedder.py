...
class Embedder:
    ...
    def __init__(self, cache_size: int = config.settings.embedding_cache_size) -> None:
        from fastembed import TextEmbedding

        if Embedder._model is None:
            log.info(
                "dense_embedding_model_loading",
    ...
    def embed_documents(self, documents: list[str]) -> list[list[float]]:
        ...
        if not documents:
            return []
        truncated = [doc[: config.settings.embedding_max_chars] for doc in documents]
