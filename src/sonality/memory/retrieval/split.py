...
def _decompose(query: str) -> _DecompositionResponse:
    ...
    log.info(
        ...
        aggregation_strategy=result.value.aggregation_strategy,
    )
...
async def split_retrieve(
    ...
    raw = await asyncio.gather(*(search_one(sq) for sq in sub_queries), return_exceptions=True)
    sub_results: list[list[EpisodeNode]] = []
    for i, r in enumerate(raw):
        if isinstance(r, BaseException):
            log.warning("subquery_search_failed", query_preview=sub_queries[i][:80], error=str(r))
        else:
